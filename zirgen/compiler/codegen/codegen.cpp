...
void addWgslSyntax(CodegenOptions& opts) {
  ...
  opts.addOpSyntax<Zll::NegOp>([](CodegenEmitter& cg, Zll::NegOp op) {
    cg << "neg_0(" << op->getOperand(0) << ")";
  });

  opts.addOpSyntax<Zll::GetOp>([](CodegenEmitter& cg, Zll::GetOp op) {
    cg << "load_at(" << op.getOffset() << "u, " << op.getBuf() << ", " << op.getBack()
       << "u)";
  });

  opts.addOpSyntax<Zll::GetGlobalOp>([](CodegenEmitter& cg, Zll::GetGlobalOp op) {
    cg << "load_at(" << op.getOffset() << "u, " << op.getBuf() << ", 0u)";
  });

  opts.addOpSyntax<Zll::SetOp>([](CodegenEmitter& cg, Zll::SetOp op) {
    cg << "store_at(" << op.getOffset() << "u, " << op.getBuf() << ", " << op.getIn()
       << ")";
  });

  opts.addOpSyntax<Zll::SetGlobalOp>([](CodegenEmitter& cg, Zll::SetGlobalOp op) {
    cg << "store_at(" << op.getOffset() << "u, " << op.getBuf() << ", " << op.getIn()
       << ")";
  });
...
void optimizeWgslStep(ModuleOp module) {
  PassManager pm(module.getContext());
  ...
  if (failed(pm.run(module))) {
    throw std::runtime_error("Failed to apply WGSL stage passes");
  }
}
...
public:
  ...
  void emitWgslStep(const std::string& stage, func::FuncOp func) {
    auto ofs = openOutputFile("step_" + stage + ".wgsl");
    *ofs << "// AUTO-GENERATED by zirgen EDSL WGSL spike. Do not edit.\n";
    codegen::CodegenEmitter emitter(codegen::getWgslCodegenOpts(), ofs.get(), func->getContext());
    emitter.emitTopLevel(func);
  }
  ...
  void emitAllLayouts(mlir::ModuleOp op) {
    ...
    emitLayout(op, codegen::getWgslCodegenOpts(), ".wgsl.inc");
  }
...
void emitCode(ModuleOp module, const EmitCodeOptions& opts) {
  ...
  for (auto [stageIndex, stage] : llvm::enumerate(stepsAttr.getSteps())) {
    ...
    if (stageName == "compute_accum" || stageName == "verify_accum") {
      optimizeWgslStep(moduleCopy);
      moduleCopy.walk([&](func::FuncOp func) {
        emitter.emitWgslStep(outputFile, func);
      });
    }
  }
